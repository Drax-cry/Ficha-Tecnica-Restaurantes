...
public class RecipeCategoryRepository : IRecipeCategoryRepository
{
    ...
    public RecipeCategoryRepository(string connectionString)
    {
    ...
    public async Task<IReadOnlyList<RecipeCategory>> GetCategoriesAsync(int userId, CancellationToken cancellationToken = default)
    {
        const string query = @"SELECT id, user_id, name, description, icon_key, color, display_order, is_active, created_at, updated_at
    ...
    public async Task<RecipeCategory> CreateCategoryAsync(RecipeCategory category, CancellationToken cancellationToken = default)
    {
        if (category is null)
        {
        ...
        const string commandText = @"INSERT INTO recipe_categories (user_id, name, description, icon_key, color, display_order, is_active)
                                     VALUES (@user_id, @name, @description, @icon_key, @color, @display_order, @is_active);";
